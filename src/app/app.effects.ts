...
export class AppEffects {
  ...
  showSideDishDialog: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
        ofType(pickSideDish),
        tap((action) => {
          if (action.item.sideDish !== undefined) {
            this.dialogRef = this.SideDishDialog.open(PickSideDishComponent, {
              data: { dish: action.item },
            });
          } else {
            this.#store.dispatch(
              addToOrderWithoutSideDish({ item: action.item })
            );
          }
        })
      ),
  ...
  showHeatDialog: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
        ofType(pickHeat),
        tap((action) => {
          this.dialogRef = this.HeatDialog.open(PickHeatComponent, {
            data: { dish: action.item },
          });
        })
      ),
  ...
  showOrder: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
        ofType(showOrder),
        tap((action) => {
  ...
  addToOrder: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
  ...
  changeMealFilter: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
  ...
  closeOrderDialog: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
        ofType(removeFromOrder),
        concatMap((action) =>
          of(action).pipe(withLatestFrom(this.#store.pipe(select(selectOrder))))
        ),
  ...
  confirmOrder: Observable<Action> = createEffect(() =>
    this.#actions.pipe(
      ...
      switchMap(([action, name, comment, order, total, method, location]) => {
        ...
        if (action.phone) {
          message = message + 'Telefon: ' + action.phone + '\n';
        }
        if (comment) {
          message = message + '\nKomentář: ' + comment;
        }
  ...
  sendOrderAnalytics = createEffect(
    () =>
      this.#actions.pipe(
        ofType(confirmOrder),
        concatMap((action) =>
  ...
  OrderSuccess: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
        ofType(OrderSuccess),
        tap(() => {
  ...
  OrderFailed: Observable<Action> = createEffect(
    () =>
      this.#actions.pipe(
        ofType(OrderFailed),
        tap((action) => {
